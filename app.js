...
{
    id:3,nombre:"Parrillada Completa",precio:5000
},
{
    id:4,nombre:"Parrillada media",precio:2500
}
];
...
{
    const transformarAEtiqueta = objeto =>
        `
    ...
    const transformarEtiquetasDeOrden = (
        objeto
    ) => `  <div class="carta_de_ordenes">
                <h3>${objeto.nombre}${objeto.precio}</h3>
            </div>`;

        const modificarArrayDePlatos = modificarArray(transformarAEtiqueta)
...
const agregarAOrden = (item) => (array) => {
   ...
   const obtener45 = obtenerPropina(45);
    ...
    if (e.target.id==="agregar_a_orden"){
        ...
        console.log(encontrarPlato(nombreDelPlato));
    }
};
